...
def dataframe_summary():
    """
    Calculate means, medians, and standard deviations for each numeric column
    in ingesteddata/finaldata.csv. Returns a flattened list:
    [mean_col1, median_col1, std_col1, mean_col2, median_col2, std_col2, ...]
    """
    ...
    for col in num_df.columns:
        stats.extend([
            float(num_df[col].mean()),
            float(num_df[col].median()),
            float(num_df[col].std(ddof=1)) if len(num_df[col].dropna()) > 1 else float('nan')
        ])
    return stats
